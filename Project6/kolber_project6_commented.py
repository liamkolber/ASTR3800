...
'''
David: Remember to use xlim and ylim to set your plot ranges and remove white space.
'''
...
'''
David: Remember to print this to screen so I know what value you got.
'''
...
def radiusDensityCalculator(It,sigIt,Inon,sigInon,number):
	...
	'''
	David: Be careful about dividing an integer by an integer. Python2.7
	interpreted 4/3 as 1, giving you the wrong numbers.
	'''
	#put it together
...
Submitted an external function file.
Code created all necessary plots.

Logic and Readability: (10/10)
Code includes descriptive comments.
Code is well spaced to aid readability.
Functions are commented.
Plots are formatted well with appropriate labels and ranges.
Code uses descriptive variable names.
Screen output formatted for reading.
Error propagation equations were correct.
Be careful about dividing an integer by an integer.

Efficiency: (5/5)
Used array syntax instead of loops where appropriate.
Functions written for repetitive tasks.
Times and shifts of transits determined algorithmically rather than manually.
