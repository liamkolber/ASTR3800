...
Performed all chi-squared tests and calculated p-values.
Made plots using interpolation with nearest values. (+1)

Logic and Readability: (10/10)
Plots are formatted well with white-space removed and good labels.
Code includes descriptive comments.
Code is spaced to aid readability.
Frequencies calculated correctly.
Chi-squared test performed correctly.
Power spectrum was normalized correctly.

Efficiency: (5/5)
Array operations used in place of loops when possible.
Repetitive tasks placed in functions.
